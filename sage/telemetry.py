...
class TelemetryEvent:
    ...
    timestamp: datetime
    duration: Optional[float] = None  # For timing measurements
    properties: Optional[Dict[str, Any]] = None  # Additional context
...
class RotatingTelemetryLogger:
    ...
    def __init__(self, log_dir: Union[str, Path], max_bytes: int = 10*1024*1024, backup_count: int = 5):
    ...
    def log_event(self, event_type: EventType, duration: Optional[float] = None, 
...
_telemetry_logger: Optional[RotatingTelemetryLogger] = None


def init_telemetry(log_dir: Union[str, Path]) -> RotatingTelemetryLogger:
...
def get_telemetry() -> Optional[RotatingTelemetryLogger]:
    """Get the global telemetry instance."""
    return _telemetry_logger


def log_event(event_type: EventType, duration: Optional[float] = None, 
              properties: Optional[Dict[str, Any]] = None):
...
def log_error(error_msg: str, context: Optional[Dict[str, Any]] = None):
